...
const nodeConf = {
    fillColor: 'rgb(128, 194, 216)',
    strokeColor: 'rgb(128,194,216)',
    strokeWidth: 3,
    textFillColor: '#fff',
    radius: 36
};
...
function initialize(resp) {
    ...
    const linkLine = container.selectAll('.link')
        ...
        .style('stroke', link => lineConf.strokeColor);
    ...
    nodeCircle.append('circle')
        ...
        .style('fill', node => nodeConf.fillColor)
        .style('stroke', node => nodeConf.strokeColor)
        .style('stroke-width', node => nodeConf.strokeWidth)
        .attr('class', 'node-circle')
        .attr('id', node => 'node-circle-' + node.id)
        .attr('r', node => nodeConf.radius);
...
//Ajax
function send(){
    myData  = {type:1,author:"byn"}
    $.ajax({//这个ajax可以在点击某个地方的时候触发
            url: "./force.php",//url 就是你的对应的php文件的相对路径
            type: 'POST', //这个不用改
            async: false,//不变
            contentType: 'application/json;charset=utf-8',//不变
            dataType: 'json',//不变 
            data: JSON.stringify(myData),//这里的mydata就是前端请求的内容比如：{page：“1” type：“1”  keyword：“bynsb”  year：“2020”}这就是前端发给php的json 最好写一个函数组织好这个json
            
